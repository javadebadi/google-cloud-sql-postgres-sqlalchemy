...
def ci(c: Context, path: str = ".", env: str = "TEST") -> None:
    ...
    Includes: autoformat, check, ty, mypy, security, complexity, test.
    """
    autoformat(c, path=path)
    check(c, path=path)
    ty(c, path=path)
    mypy(c, path=path)
    # Security and complexity use their own default paths (server)
    security(c)
    complexity(c)
...
ns.add_task(deadcode)
ns.add_task(docstrings)
ns.add_task(typecov)
ns.add_task(licenses)
ns.add_task(duplication)
ns.add_task(clean)
